function initializeHumedad(dataJson){
	...
	// define the area
	var area = d3.area()
		.defined(function(d) { return (d.hume !== null) })
	    .x(function(d) { return x(d.time); })
	    .y0(height)
	    .y1(function(d) { return y(d.hume); });

	// define the line
	var valueline = d3.line()
		.defined(function(d) { return (d.hume !== null) })
	    .x(function(d) { return x(d.time); })
	    .y(function(d) { return y(d.hume); });

	// append the svg obgect to the body of the page
	// appends a 'group' element to 'svg'
	// moves the 'group' element to the top left margin
	var svg = d3.select('#humedadChart').append("svg")
	...
	var miny = d3.min(data, function(d) { return d.hume; });
	var maxy = d3.max(data, function(d) { return d.hume; });
	...
	svg.append("path")
	   	.data([data])
	   		.attr("class", "areaHumedad")
	   		.attr("d", area);
	
	// add the valueline path.
	svg.append("path")
	  	.data([data])
	  		.attr("class", "lineHumedad")
	  		.attr("d", valueline);
